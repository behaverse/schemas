...
- The divergent `behaverse:` prefix (bound per family to that family's own namespace, and unused here) is removed from the source and the published `context.jsonld`. No term URIs changed; `event:` and `bdm:` remain.
