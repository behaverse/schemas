...
def convert_linkml_to_fields(linkml: Dict[str, Any]) -> List[Dict[str, Any]]:
    """Convert LinkML schema classes to field definitions"""
    fields = []
    classes = linkml.get('classes', {})
    ...
    for class_name, class_def in classes.items():
        ...
        if attributes:
            ...
            for attr_name, attr_def in attributes.items():
                attr_field = {
                    'name': attr_name,
                    'type': attr_def.get('range', 'string'),
                    'description': attr_def.get('description', ''),
                    'requirement': 'required' if attr_def.get('required', False) else 'optional',
                }
                
                # Add default/ifabsent value
                if 'ifabsent' in attr_def:
                    attr_field['default'] = attr_def['ifabsent']
                
                # Add multivalued/array indicator
                if attr_def.get('multivalued', False):
                    attr_field['type'] = 'array'
                
                # Add identifier indicator
                if attr_def.get('identifier', False):
                    attr_field['constraint'] = {'pattern': '^[a-zA-Z0-9_-]+$'}
                
                # Add conditional logic from extensions
                if 'extensions' in attr_def:
                    ext = attr_def['extensions']
                    if 'condition_body' in ext:
                        attr_field['conditional'] = ext['condition_body']
                
                field['properties'].append(attr_field)
    ...
    if schema_info['has_field_definitions']:
        ...
        return {
            'metadata': {
                ...
                'version': linkml.get('version', 'v25.1217.dev2'),
                'namespace': linkml.get('id', 'http://behaverse.org/schemas/studyflow'),
                'description': linkml.get('description', 'LinkML schema for Behaverse Studyflow'),
            },
